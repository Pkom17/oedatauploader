...
public class Task {
	...
	public void readVlAnalysisForInsert() {
		while (true) {
			try {
				...
				logger.error("Task - readVlAnalysisForInsert", ex);
				break;
			}
		}
	}
	...
	public void pushInsertData() {
		...
		while (!isInsertTaskOver) {
			try {
				List<VlAnalysisRecord> dataToPushInsert = vlService
						.getAnalysisToPush(VlAnalysisRecord.STATUS_TO_INSERT);
				...
				logger.error("Task - pushInsertData", ex);
				break;
			}
		}
	}
	...
	public void pushUpdateData() {
		...
		while (!isUpdateTaskOver) {
			try {
				List<VlAnalysisRecord> dataToPushUpdate = vlService
						.getAnalysisToPush(VlAnalysisRecord.STATUS_TO_UPDATE);
				...
				logger.error("Task - pushUpdateData", ex);
				break;
			}
		}
